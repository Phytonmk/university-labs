...
std::deque<monomial> getPolynomialByString(std::string input) {
  std::deque<monomial> polynomial;
...
FinalFunction convertPolynomialToFunction(std::deque<monomial> polynomial) {
...
std::deque<int> calculateFunction(FinalFunction finalFunction) {
  std::deque<float> floatChart;
...
void drawChart(std::deque<int> chart) {
  initscr();
  cbreak();
  noecho();
